...
model_smoke = torch.hub.load('./yolov5', 'custom', path='./model/best.pt', source='local')
...
class VideoTransformTrack(MediaStreamTrack):
    ...
    async def recv(self):
        ...
        if self.transform == "cartoon":
            ...
            img = frame.to_ndarray(format="bgr24")
            results = model_smoke(img)
            new_frame = VideoFrame.from_ndarray(results[0].plot(), format="bgr24").to_image()
            new_frame.pts = frame.pts
            new_frame.time_base = frame.time_base
            return new_frame
...
def create_local_tracks(play_from=None):
    if play_from:
        ...
        options = {"framerate": "0.1", "video_size": "640x480"}
        webcam = MediaPlayer(
